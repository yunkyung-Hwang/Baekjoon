...
for i in 1...n {
    ...
    for j in 1...m {
        area[i][j] = tmp[j-1] + area[i][j-1] + area[i-1][j] - area[i-1][j-1]
    }
}
let k = Int(readLine()!)!
...
for k in 0..<k {
    print(area[range[k][2]][range[k][3]] - area[range[k][2]][range[k][1]-1] - area[range[k][0]-1][range[k][3]] + area[range[k][0]-1][range[k][1]-1])
}
